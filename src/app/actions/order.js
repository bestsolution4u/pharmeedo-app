...
export const addOrder = (params) => (dispatch) => {
  ...
  return OrderServices.addOrder(params)
    .then(({data}) => {
      dispatch({type: ActionTypes.ADD_ORDER_SUCCESS, data});
    })
    .catch((message) => {
    ...
    orders.map(async (order) => {
      try {
        const items = order.items.map((item) => {
          ...
          if (productId.includes('local-')) {
            ...
            const newProduct = allProducts.filter(
              (product) => product.localId === productId,
            );
        ...
        if (customerId && customerId.includes('local-')) {
          ...
          const newCustomer = allCustomer.filter(
            (customer) => customer.localId === customerId,
          );
        ...
        if (order.coupon && order.coupon.includes('local-')) {
          ...
          const newCoupon = allCoupon.filter(
            (coupon) => coupon.localId === order.coupon,
          );
