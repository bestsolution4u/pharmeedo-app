...
const Dashboard = ({navigation}) => {
  ...
  const chartBackground = useDynamicValue(
  ...
  const getTopProducts = () => {
    ...
    ReportServices.getTopProducts()
      .then(({data}) => {
        const products = slice(data, 0, Constants.NUMBER_CHART_COLUMNS);
        const chartTopProductLables = products.map((product) =>
          product.name.length > Constants.MAX_LENGTH_CHART_LABEL
            ? `${product.name.substring(
                0,
                Constants.MAX_LENGTH_CHART_LABEL,
              )}...`
            : product.name,
        );
        const dataChart = products.map((product) => product.quantity);
        const chartProducts = {
  ...
  const getMonthlySale = () => {
    ...
    ReportServices.getReportMonthly()
      .then(({data}) => {
