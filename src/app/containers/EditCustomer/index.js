...
const EditCustomer = (props) => {
  ...
  const editCustomerSuccess = () => {
    ...
    const customer = navigation.getParam('data');
    const newData = customersReducer.result.filter(
  ...
  const handleEditCustomer = async () => {
    ...
    try {
      ...
      if (
        ...
        validator.isEmpty(params.first_name)
      ) {
      ...
      if (
        !validator.isEmpty(params.billing.phone) &&
      ...
      if (isOfflineMode) {
        return dispatch(
          saveCustomerEditing({...params, avatar: imageUri, status: 'editing'}),
        );
      }

      if (photo && photo.indexOf('http') !== 0) {
        const imageUploaded = await ImageServices.uploadImage({uri: imageUri});
        params.avatar = {id: imageUploaded.id};
      }
      ...
      <KeyboardAwareScrollView enableResetScrollToCoords enableOnAndroid>
        <View style={styles.mainContainer}>
          ...
          <Text style={styles.inputLabel}>
            {I18n.t('editCustomer.nameCustomer')}
          </Text>
          <TextInput
          ...
          <TouchableOpacity
            ...
            {isEditing ? (
              ...
              <Text style={styles.lbAddNew}>
                {I18n.t('editCustomer.saveCustomer')}
              </Text>
            )}
          </TouchableOpacity>
        </View>
