...
const Coupon = (props) => {
  ...
  const search = (q) => {
    const result = dataRefactor.filter((item) =>
      Utils.searchObject(item, ['code'], q),
    );
    setData(result);
  };
  ...
  const handlePressAction = ({type, item}) => {
    if (type === 'delete') {
      Alert.alert(
        'Delete confirm',
        `Are you sure you want to delete coupon: ${item.code}`,
        [
          {
            text: 'Cancel',
            style: 'cancel',
  ...
  const deleteCoupon = (item) => {
    const {removeCoupon, saveCouponDeleted} = ActionCreators;
    ...
    dispatch(ActionCreators.deleteCoupon(couponId));
  };
