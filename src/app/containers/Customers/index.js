...
const configs = [
  {header: 'customers.name', property: 'first_name&last_name'},
  {header: 'customers.phone', property: 'billing.phone'},
  {header: 'customers.email', property: 'email'},
  {header: 'customers.address', property: 'shipping.address_1'},
];
...
const Customers = (props) => {
  ...
  const handleLoadMore = () => {
    ...
    if (
      data.length === allCustomers.length ||
      isLoadingMore ||
  ...
  const search = (q) => {
    const result = allCustomers.filter((item) =>
      Utils.searchObject(
        item,
        [
          'email',
  ...
  return (
    <View style={styles.container}>
        ...
        <View style={styles.row}>
          ...
          <TouchableOpacity style={styles.btnAddNew} onPress={goToAddCustomer}>
            <Text style={styles.lbAddNew}>
              {I18n.t('customers.importCustomers')}
            </Text>
          </TouchableOpacity>
          */}
      ...
      <Table
        ...
        ListFooterComponent={
          (isLoadingMore || customersReducer.requesting) && (
