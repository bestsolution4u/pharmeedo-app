...
const AddCategory = (props) => {
  ...
  const addCategoryReducer = useSelector(
    (state) => state.categoryReducers.addCategory,
  );
  ...
  const addCategory = async () => {
    const {saveCategory} = ActionCreators;
    try {
      ...
      dispatch(ActionCreators.addCategory(params));
    } catch (error) {
      setIsAdding(false);
      dropdownAlertRef.current.alertWithType(
        'error',
        ...
        <TouchableOpacity
          ...
          {isAdding ? (
            ...
            <Text style={styles.lbAddNew}>
              {I18n.t('addCategory.addCategory')}
            </Text>
          )}
        </TouchableOpacity>
      </ScrollView>
